...
uint32 flash_image[4]={0};
uint32 flash_image_init[4]={450,0,0,15};
...
//需要配置到摄像头的数据 //正误差 给正值 、、5 =0.8 -3,2 1.6*2 越小负越靠右 4.8 0.8
int16 MT9V03X_CFG[CONFIG_FINISH][2]=
{
	...
	{LR_OFFSET,			5},	 //9可以走直												// 图像左右偏移量	正值 右偏移   负值 左偏移  列为188 376 752时无法设置偏移    摄像头收偏移数据后会自动计算最大偏移，如果超出则设置计算出来的最大偏移
	{UD_OFFSET,			0},													// 图像上下偏移量	正值 上偏移   负值 下偏移  行为120 240 480时无法设置偏移    摄像头收偏移数据后会自动计算最大偏移，如果超出则设置计算出来的最大偏移
	{GAIN,				15},												// 图像增益			范围16-64     增益可以在曝光时间固定的情况下改变图像亮暗程度
...
void set_config(UARTN_enum uartn, int16 buff[CONFIG_FINISH-1][2])
{
	...
	for(i=0; i<CONFIG_FINISH; i++)
	{
		...
		temp = buff[i][1];//1 5 6 7
                if(i==1) temp=flash_image[0]; //曝光
                if(i==5)
                {
                  temp=flash_image[1]; //左右
                }
                if(i==6) temp=flash_image[2]; //上下
                if(i==7) temp=flash_image[3]; //增益
