...
int White =210;
int Black =210;
int drath=8,drath_banma=32,sobel_yun=30;
int yidong=50;
  ...
  if(shizi==0)
  {
   // if(beepds==0) beepds=3;
    ...
    if(y==40)
    {
      ...
      //beepds=5;
      sum_mc=0;
      if(shuzi_num!=1&&shuzi_num!=2&&first_sizi==0)
      {
        first_sizi=1;
        opmv_data=0x15;
        shizi=2;
...
void into_home()
{
 ...
 int stop_Color=0;
  ...
  for(n=54;n<58;n++)
  {
    ...
    {
      if(abs(mt9v03x_image[n][m]-mt9v03x_image[n][m+1])>23)  stop_Color++;
    }
  }
    if(stop_Color>sobel_yun)
    {
      
       if(lu_sz[0]==1)  uart_putchar(UART_4,'Y');
       if(lu_sz[0]==0)  uart_putchar(UART_4,'X');
      gpio_set(green,1);
      beepds=20;
      stop_mc_flash=1;
    //  STOP();
