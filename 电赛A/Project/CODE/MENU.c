...
//   gpio_init(H2 , GPO, GPIO_HIGH, GPO_PUSH_PULL);        // 单片机LED灯
   ...
   gpio_init(bo_4, GPI, 1, GPI_PULL_UP); 
   ...
   gpio_init(bo_5, GPI, GPIO_HIGH, GPI_FLOATING_IN);
  ...
  //电机初始化
   ...
   flash_page_read(FLASH_SECTION_127,FLASH_PAGE_3,flash_image,5); //读出摄像头需要的4个数据
//    flash_page_read(FLASH_SECTION_126,FLASH_PAGE_0,&num_go_ci,1); //读出摄像头需要的4个数据
   //外设初始化
    lcd_init(); 
    icm20602_init();
    mt9v03x_init();
...
void caidan()
{
  ...
  if(opmv_data!=0&&shuzi_num==0) 
  {
    zhen_go=2;
    shuzi_num=opmv_data;                   //注意字符转换
//    gpio_set(beepd,1);
...
//    gpio_set(beepd,0);
  ...
  if(gpio_get(bo_4)==0&&zhen_go==2&&fx_fx_fx==0)
  {
...
//    lcd_showint16(75,1,(int)(stop_time));
  ...
  {    
    ...
    lcd_showstr(10,4,"num_go_ci");
    lcd_showint16(75,4,num_go_ci);
    ...
    if(key_judge(jian))
    {
     ...
     flash_page_program(FLASH_SECTION_127, FLASH_PAGE_3,flash_image,4);
     flash_page_program(FLASH_SECTION_126, FLASH_PAGE_0,&num_go_ci,1);
...
//      if(t==8) se=8;
   ...
   if(gpio_get(bo_4)==0&&fx_fx_fx==0&&zhen_go==2) 
   {
     fx_fx_fx=1;
     zhen_go=1;
     systick_delay_ms(700);
   }
    if(gpio_get(bo_5)==0&&fx_fx_fx==0&&zhen_go==2) 
   {
     fx_fx_fx=2;
     zhen_go=1;
     systick_delay_ms(700);
...
/*---------------返回最初-----------------*/    
  ...
  {
    lcd_clear(WHITE);
    if(zhen_go==2)  uart_putchar(UART_3,'S');//
    se=0;
    t=0;
  }
}
